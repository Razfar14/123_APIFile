...
async function create(req, res) {
    try {
        const {
            ...
            sinopsis,
            tahun_terbit,
            penulis_id: body_penulis_id,
            genre_ids
        } = req.body || {};

        const penulis_id = body_penulis_id || (req.penulis && req.penulis.id);

        if (!penulis_id) {
            return res.status(400).json({
                message: "Penulis ID wajib diisi."
            });
        }
...
async function update(req, res) {
    try {
        ...
        const penulis_id = body_penulis_id || (req.penulis && req.penulis.id) || komik.penulis_id;
